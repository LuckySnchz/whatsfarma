...
divce.style.display= "block";
    ...
    <script type="text/javascript">
      function productounom(sel) {

              var productounom=sel.value;
              $('#productounom').val(productounom);
             
            }
     
       function preciounom(sel) {

              var preciounom=sel.value;
              $('#preciounom').val(preciounom);
...
<section class="container jumbotron shadow p-3 mb-5 bg-white rounded"style="height:10500px;text-align: center;margin-top: -8%" >
 <form class="" action="/mujereseditarfinal" method="get" >
...
<img class="group list-group-image" src="{{ asset('img/'.$producto->image) }}" alt="" style="width: 150px;height: 150px" /> 
  ...
  @endforeach
   ...
   <b><input type="text" id="DescripcionProducto" onclick="editproducto();" name="DescripcionProducto" value="{{session('descripcion')}}"style="width: 300px;height: 50px;border:solid 0.1px white"> <br><br>



   <strong><input type="text" id="Precio" onclick="editprecio();" name="Precio"style="color:red" value="{{session('precio')}}"></strong></b>

<a type="button"target="_self" href="mujereseditarfinal/{{session('id')}}/{{'DescripcionProducto'}}/{{'Precio'}}" style="width:100%;
